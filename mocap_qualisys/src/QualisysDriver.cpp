...
bool QualisysDriver::init() {
  ...
  // Get 6DOF settings
  bool readStatus;
  port_protocol.Read6DOFSettings(readStatus);

  marker_publisher_ = nh.advertise<qualisys_msgs::Markers>("markers", 1000);
...
void QualisysDriver::run() {
  ...
  port_protocol.GetCurrentFrame(CRTProtocol::cComponent6dEuler | CRTProtocol::cComponent3dNoLabels);
...
void QualisysDriver::handleFrame() {
  ...
  int marker_count = prt_packet->Get3DNoLabelsMarkerCount();
  qualisys_msgs::Markers markerList;
  markerList.header.stamp = ros::Time::now();
  markerList.header.frame_id = "mocap";

  for (int i = 0; i < marker_count; ++i) {
    float x, y, z;
    unsigned int id;
    prt_packet->Get3DNoLabelsMarker(i, x, y, z, id);
    qualisys_msgs::Marker marker;
    marker.point.x = x/1000;
    marker.point.y = y/1000;
    marker.point.z = z/1000;
    marker.index.data = id;
    markerList.markers.push_back(marker);
  }

  marker_publisher_.publish(markerList);
